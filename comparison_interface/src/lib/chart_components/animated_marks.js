...
// Source: https://github.com/cmudig/emblaze/blob/main/src/visualization/models/data_items.js

import { approxEquals, makeTimeProvider } from "$lib/utils";
...
export class LineDecoration extends Decoration {
  constructor(mark, mark2, lineWidthFn, lineAlphaFn) {
    super("line", [mark, mark2], {
      lineWidth: {
...
export function MarkSet(marks, decorations = null) {
  ...
  this.advance = function (dt) {
    ...
    if (
      animatingMarks.size == 0 &&
      animatingDecorations.size == 0 &&
      !visibleMarksChanged
    )
      return false;
  ...
  this.animateOne = function (id, attrName, animator) {
    if (!this.marksByID.get(id)) {
      console.error(`mark with id ${index} not found`);
      return;
    }
    let mark = this.marksByID.get(id);
    animatingMarks.add(mark);
    updatedMarks.add(mark);
    mark.animate(attrName, animator);
  };
  ...
  this.setOne = function (id, attrName, value) {
    if (!this.marksByID.get(id)) {
      console.error(`mark with id ${index} not found`);
      return;
    }
